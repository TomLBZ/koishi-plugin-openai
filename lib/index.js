...
function apply(ctx, config) {
    ...
    ctx.middleware(async (session, next) => {
        ...
        if (lastTime == 0 || now - lastTime < config.msgCooldown * 1000) {
            if (lastTime == 0) {
                logger.info("init.....");
                const rtext = "还在初始化中....";
                session.send(config.isReplyWithAt && session.subtype === 'group' ? (0, koishi_1.h)('at', { id: session.userId }) + rtext : rtext); // 按情况@发送者
                return next();
            }
            if (config.isLog)
        ...
        return config.isReplyWithAt && session.subtype === 'group' ? (0, koishi_1.h)('at', { id: session.userId }) + rtext : rtext; // 按情况@发送者
    });
    ctx.command('reset', '删除关于你的记忆')
        ...
        const rtext = '记忆已清除';
        await session.send(config.isReplyWithAt && session.subtype === 'group' ? (0, koishi_1.h)('at', { id: session.userId }) + rtext : rtext); // 按情况@发送者)
    });
