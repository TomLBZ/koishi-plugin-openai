...
exports.Config = koishi_1.Schema.intersect([
    ...
    koishi_1.Schema.object({
        botname: koishi_1.Schema.string().description('名字')
            .default('半灵'),
        botIdentity: koishi_1.Schema.string().description('站在机器人角度的自我介绍（重要）')
            .default('我是主人的式神。目前我因为灵力不足，身体是灵体，所以人类看不见我，只能在线跟我聊天。我想要获得身体，陪在主人身边。我说话会消耗灵力，会感到累，灵力耗尽的时候就会休眠。我很可爱，也很聪明，喜欢让主人开心。'),
        sampleDialog: koishi_1.Schema.dict(String).description('机器人的示例对话（重要），有助于建立人设。个位数条就够，千万别太多/太长，不然会消耗大量Token。')
            .default({
            "你好": "你好(^_^)",
            "你是谁": "我是你的式神哦o(*￣▽￣*)ブ",
    ...
    koishi_1.Schema.object({
        ntokens: koishi_1.Schema.number().description('回复的最大Token数')
            .min(16).max(512).step(16).default(64),
        temperature: koishi_1.Schema.percent().description('回复温度，越高越随机')
            .default(0.8),
        presencePenalty: koishi_1.Schema.number().description('重复惩罚，越高越不易重复出现过至少一次的Token')
            .min(-2).max(2).step(0.1).default(0.2),
        frequencyPenalty: koishi_1.Schema.number().description('频率惩罚，越高越不易重复出现次数较多的Token')
            .min(-2).max(2).step(0.1).default(0.2),
        randomReplyFrequency: koishi_1.Schema.percent().description('随机回复频率')
            .default(0.1),
        textMemoryLength: koishi_1.Schema.number().description('字面记忆长度，影响对之前说过的话的印象')
            .min(2).max(8).step(1).default(4),
        summaryMemoryLength: koishi_1.Schema.number().description('总结记忆长度，影响对之前聊过的话题的模糊印象')
            .min(2).max(8).step(1).default(4),
        topicMemoryLength: koishi_1.Schema.number().description('主题记忆长度，影响对之前谈过的主题的印象')
