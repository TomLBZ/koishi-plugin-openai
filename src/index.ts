...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.middleware(async (session, next) => {
    ...
    const t: Timing = {
      start: 0,
      openai: 0, // openai
      wolfram: 0, // wolframalpha
      pinecone: 0, // pinecone
      search: 0, // google / bing
      cache: 0, // cache
    }
    // ======== 处理输入向量 ========
    t.start = Date.now()
    ...
    if (config.isLog) logger.info(`Input Keywords: ${JSON.stringify(ikeywords)}`) // 打印输入的关键词
    ...
    t.start = Date.now()
    if (hint && hint.length > 0) { // 计算结果不为空，无需用其他任何知识，因为Wolfram永远正确
      if (config.isLog) logger.info(`Knowledge Mode: WolframAlpha: ${hint}`)
      ...
      if (config.isLog) logger.info(`Memory Keywords: ${JSON.stringify(mkeywords)}`)
      let info: string[] = [] // 用于存储检索结果
      t.pinecone += Date.now() - t.start
      t.start = Date.now()
      if (mkeywords && mkeywords.length > 0) { // 如果有新关键词，可以检索数据库
        ...
        if (config.isLog) logger.info(`Pinecone found ${info.length} matches`)
        t.pinecone += Date.now() - t.start
        t.start = Date.now()
      } // 如果没有新关键词，或者检索结果不足，需要上网搜索
      if (!info || info.length < config.searchTopK) {
        if (ikeywords && ikeywords.length > 0) { // 如果输入有关键词，可以搜索
          const webres = await soul.search(input, ctx) // 搜索原始输入
          if (webres && webres.length > 0) { // 如果搜索结果不为空，可以加入info
            if (config.isLog) logger.info(`Knowledge Mode: ${soul.searchMode}`)
            info.push(...webres) // 将搜索结果添加到info中
          }
        }
        t.search += Date.now() - t.start
        t.start = Date.now()
      } else if (config.isLog) logger.info(`Knowledge Mode: Long-term Memory`)
      const usedinfo = info.slice(0, config.searchTopK) // 保留前K个结果
      if (usedinfo && usedinfo.length > 0) { // 如果有记忆或搜索结果，可以生成主会话
    ...
    if (config.isLog) logger.info(`Reply Keywords: ${JSON.stringify(rkeywords)}`)
