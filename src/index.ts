import { Context, Logger, Middleware, Session, h } from "koishi";
...
export function apply(ctx: Context, config: Config) {
  const replyMessage = async (
    ...
    session.send(
      config.isReplyWithAt && session.subtype === "group"
  ...
  ctx.on("ready", async () => {
    ...
    if (config.isLog)
      logger.info(
        `Initialization: AI(${bai ? "√" : "X"}) Soul(${
          bsoul ? "√" : "X"
        }) Eye(${beye ? "√" : "X"}) Cache(${bcache ? "√" : "X"})`
      );
    lastTime = Date.now();
  });
  ...
  ctx.middleware(async (session, next) => {
    ...
    const now = Date.now();
    if (lastTime == 0) {
      if (config.isLog) logger.info("init.....");
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      const replyText = `余额：$${balance.total_used.toFixed(
        ...
        (balance.total_used / balance.total_granted) *
        100
      ).toFixed(2)}%`;
