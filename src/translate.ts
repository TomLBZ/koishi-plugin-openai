...
export class Translate {
  ...
  public async init(
    ...
    this.mode = (await this.testTransl(context))
      ? "Google"
      : this._azureKey
      ? "Bing"
      : "None"; //'baidu'
  ...
  private async testTransl(context: Context): Promise<boolean> {
    const url =
      "https://translate.google.com/translate_a/single?client=gtx&sl=auto&tl=zh-CN&dt=t&q=Who+is+Tom";
    try {
  ...
  private async _googleTranslate(
    query: string,
    tlang: string,
    context: Context
  ): Promise<string> {
