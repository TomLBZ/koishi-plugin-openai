...
export class Search {
  ...
  private _reduceElement(elem: Element, islenfilter: boolean = false): void {
  ...
  private _keepCommonClass(elem: Element): void {
  ...
  private _checkClassName(elem: Element, classnames: string[]): boolean {
    for (const classname of classnames)
      if (
        elem.className.includes(classname) ||
  ...
  private _checkClassNames(
    ...
    return (
      this._checkClassName(elem.children[0], names) &&
